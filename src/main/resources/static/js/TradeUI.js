...
function renderTradeWindow(otherPlayerPokemon, otherPlayerCoins, otherPlayerId) {
	// initialization
	...
	// panel
	let tradePanel = new SlickUI.Element.Panel(Game.map.widthInPixels/6, Game.map.heightInPixels/25, Game.map.widthInPixels/1.5, Game.map.heightInPixels/2.3);
	...
	acceptTradeButton.events.onInputDown.add(function () {
    	// accept trade
    	let pokemonIds = yourPokemonOffer.map(pokemon => pokemon.id);
	...
	renderTradeWindowHelper(0, yourPokemonOffer, yourPanel, team, pokemonSelected, true, otherPlayerCoins, otherPlayerId);
	renderTradeWindowHelper(0, yourPokemonOffer, otherPanel, otherPlayerPokemon, pokemonSelected, false, otherPlayerCoins, otherPlayerId);
}
...
function renderTradeWindowHelper(startIndex, selectedPokemon, panel, team, pokemonSelected, yourMenu, otherPlayerCoins, otherPlayerId, otherPlayerPokemon) {
	...
	if (yourMenu) {
    	...
    	// scroll down
    	if (startIndex+tradeWindowRowSize*tradeWindowColumnSize+1 < team.length) {
	...
	for (let i=0; i<tradeWindowColumnSize; i++) {
		for (let k=0; k<tradeWindowRowSize; k++) {
			if (tradeWindowColumnSize*tradeWindowRowSize*i + k + startIndex < team.length) {
